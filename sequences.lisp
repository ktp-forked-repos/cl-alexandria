...
(defun shuffle-sublist (list n)
  (when (> n 1)
    (let* ((half (floor n 2))
      ...
      (shuffle-sublist b (- n half))
      (do ((a list)
           (remaining-total n (1- remaining-total))
           (remaining-a half))
          ((= remaining-a 0))
        (if (< (random remaining-total) remaining-a)
...
sequence."
  ...
  (let* ((length (length sequence))
    ...
    (etypecase sequence
      (list
       (shuffle-sublist (nthcdr start sequence) (- end start)))
      (vector
